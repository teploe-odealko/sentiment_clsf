...
def generate_bow(preprocessed, params):
    vectorizer = CountVectorizer(token_pattern=r'[A-Za-zА-Яа-я]+',
                                 min_df=params['min_vectorizer_freq'])
    return vectorizer.fit_transform(preprocessed.text).toarray()


def generate_tfidf(preprocessed, params):
    vectorizer = TfidfVectorizer(token_pattern=r'[A-Za-zА-Яа-я]+',
                                 min_df=params['min_vectorizer_freq'])
    return vectorizer.fit_transform(preprocessed.text).toarray()


def generate_w2v(preproccessed, params):
    w2v_model = Word2Vec(min_count=params['w2v']['min_count'],
    ...
    w2v_model.build_vocab(preproccessed.text.apply(lambda x: x.split()), progress_per=1000)
    w2v_model.train(preproccessed.text.apply(lambda x: x.split()),
...
def generate_features(preprocessed: pd.DataFrame, params: Dict) -> pd.DataFrame:
    ...
    print(type(w2v_features))
    print(bow_features.shape)
    print(w2v_features.shape)
    stacked_features = np.concatenate([bow_features, tfidf_features, w2v_features], axis=1)
